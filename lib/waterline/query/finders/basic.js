...
module.exports = {
  ...
  find: function(criteria, options, cb) {
    ...
    // Add join_sql & where_sql values to criteria
    if (options && options.join_sql) {
      criteria.join_sql = options.join_sql;
    }
    if (options && options.where_sql) {
      criteria.where_sql = options.where_sql;
    }
    // Add distinct & select_sql values to criteria
    if (options && options.distinct) {
      criteria.distinct = options.distinct;
    }
    if (options && options.select_sql) {
      criteria.select_sql = options.select_sql;
    }
